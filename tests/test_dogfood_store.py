...
def test_log_dogfood_source_roundtrip(store):
    ...
    round_tripped = store.get(dog.id)
...
def test_list_filters_by_kind(store):
    ...
    ids = {d.id for d in store.list(kind=DOGFOOD_KIND_FRICTION)}
    assert ids == {f.id}
...
def test_list_filters_by_target(store):
    ...
    ids = {d.id for d in store.list(target="developer")}
    assert ids == {b.id}
...
def test_list_filters_by_since(store):
    ...
    ids = [d.id for d in store.list(since=50.0)]
    assert ids == [b.id]
...
def test_list_status_all_includes_terminal(store):
    ...
    b = store.log_dogfood("dismissed", observed_at=2.0)
    store.mark_dismissed(b.id)
    ids = {d.id for d in store.list(status="all")}
    assert a.id in ids
    assert b.id in ids
...
def test_list_filters_by_status_name(store):
    ...
    store.mark_dismissed(b.id)
    ids = [d.id for d in store.list(status=DOGFOOD_STATUS_DISMISSED)]
    assert ids == [b.id]
...
def test_seed_data_present_on_first_init(tmp_path):
    ...
    dogs = store.list(status="all", limit=None)
    assert len(dogs) == 5
...
def test_seed_data_idempotent_on_second_init(tmp_path):
    ...
    first_count = len(
        DogfoodStore(knowledge=knowledge_a, seed=False).list(status="all", limit=None)
    )

    knowledge_b = KnowledgeStore(db_path)
    DogfoodStore(knowledge=knowledge_b)  # re-seed attempt
    second_count = len(
        DogfoodStore(knowledge=knowledge_b, seed=False).list(status="all", limit=None)
    )
...
def test_seed_data_skipped_when_store_already_has_rows(tmp_path):
    ...
    assert len(store_seeded.list(status="all", limit=None)) == 1
